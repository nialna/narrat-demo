        ...
        ? (openBlock(), createBlock("div", _hoisted_2, [
            (openBlock(true), createBlock(Fragment, null, renderList(_ctx.choices, (choice, index) => {
              return (openBlock(), createBlock("p", {
                key: index,
                onClick: $event => (_ctx.chooseOption(index)),
                class: "dialog-choice"
              }, toDisplayString(index + 1) + ". –  " + toDisplayString(choice.choice), 9, ["onClick"]))
            }), 128))
          ]))
        : (_ctx.canInteract)
...
const _hoisted_3$1 = { class: "dialog-container" };
const _hoisted_4 = { key: 1 };
const _hoisted_5 = /*#__PURE__*/createVNode("option", { value: "data/easy-data.rpy" }, "Select an option", -1);
      ...
      : (_ctx.gameLoaded)
        ? (openBlock(), createBlock("div", _hoisted_4, [
            createVNode("select", {
              ...
              onChange: _cache[1] || (_cache[1] = $event => (_ctx.scriptSelected($event)))
            }, [
              _hoisted_5,
              (openBlock(true), createBlock(Fragment, null, renderList(_ctx.games, (game, key) => {
...
function processSkillCheck(state, skillcheck) {
...
function runSkillCheck(state, params) {
...
function runConditionCommand(state, command) {
...
function runCondition(state, condition) {
    return conditionFunction(state, condition);
}
function conditionFunction(state, condition) {
...
async function runCommand(context, cmd, choices) {
    ...
    switch (cmd.commandType) {
        ...
        case 'if':
            const newBranch = runConditionCommand(state, cmd);
            if (newBranch) {
                const newStack = {
                    branch: newBranch,
                    currentIndex: 0,
...
async function playerAnswered(context, choiceIndex) {
    ...
    switch (cmd.commandType) {
        case 'choice':
            ...
            if (skillcheck) {
                const result = processSkillCheck(state, skillcheck);
...
async function runChoice(context, cmd) {
    ...
    const choices = options.choices.filter(choice => {
        ...
        if (choice.condition) {
            return runCondition(state, choice.condition);
        }
        return true;
    }).map(choice => {
...
const INDENT_SIZE = 4;
function processRenpyCommands(lines) {
    let currentLine = 0;
    const branch = [];
    while (currentLine < lines.length) {
        ...
        switch (operator) {
            ...
            case 'choice':
                ...
                const prompts = choices.map(choice => {
...
function parseRenpyScript(code) {
...
function findRenpyBranches(code, startLine, indentLevel) {
...
function validateIndent(indentLevel, currentIndex) {
    if (indentLevel % 1 !== 0) {
